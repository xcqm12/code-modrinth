...
Server Projects are coming to Bbsmc, and they’re not just a typical server list. They’re deeply integrated into the platform and app in ways that make playing seamless.
...
Additionally, modpack discovery has become noisy on Bbsmc. Servers fork popular packs with small tweaks and climb discovery, which crowds out original creators.
...
- Vanilla server
- Modded server (published Bbsmc pack)
- Modded server (uploaded custom pack)

Each type defines different requirements to join. Vanilla servers specify supported and recommended Minecraft versions. Modded servers either link a Bbsmc pack or upload a custom pack, which enforces the required version and mod loader.
...
- **Verified plays:** Joins from the Bbsmc App in the last two weeks.
