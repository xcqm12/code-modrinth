...
- Set the modalâ€™s width via the `width` or `maxWidth` props. For responsive sizing, use `min(base-size, calc(95vw - 10rem))`.
- `ModalWrapper` is deprecated â€?modal behavior is automatically handled via the `injectModalBehavior` DI utility.

## Basic Usage

```vue
<script setup lang="ts">
import { ref } from â€˜vueâ€?import { NewModal } from â€˜@Bbsmc/uiâ€?
...
| --------------------- | ------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------ |
| `header`              | `string`                              | â€?            | Title text displayed in the header bar                                                           |
...
| `maxContentHeight`    | `string`                              | `â€?0vhâ€™`      | Max height of the scrollable content area (only applies when `scrollable`)                       |
| `noPadding`           | `boolean`                             | `false`       | Removes padding from the content area for edge-to-edge layouts                                   |
| `maxWidth`            | `string`                              | `â€?0remâ€™`     | Maximum width of the modal                                                                       |
| `width`               | `string`                              | `fit-content` | Width of the modal body                                                                          |
| `noblur`              | `boolean`                             | â€?            | Disables backdrop blur. Defaults to the value from `injectModalBehavior`                         |
| `fade`                | `â€™standardâ€?\| â€˜warningâ€?\| â€˜dangerâ€™` | `â€™standardâ€™`  | Overlay color variant                                                                            |
| `danger`              | `boolean`                             | `false`       | **Deprecated** â€?use `fade="danger"` instead                                                     |
| `onShow`              | `() => void`                          | â€?            | Called when the modal opens                                                                      |
| `onHide`              | `() => void`                          | â€?            | Called when the modal closes                                                                     |
...
The `checkScrollState` method is exposed via ref â€?call it after dynamically changing content to re-evaluate whether fade indicators should appear.
...
`NewModal` integrates with a modal stack (`useModalStack`). Multiple modals can be open simultaneously â€?only the topmost modal responds to the Escape key. The document body scroll is locked when any modal is open and restored when the last modal closes.
...
1. **Context** â€?A DI provider that holds all state, business logic, and stage configs
2. **Stage configs** â€?Data objects describing each stage (title, component, buttons, skip conditions)
3. **Stage components** â€?Vue components rendered inside the modal, consuming the context
...
Navigate to it with `modal.value?.setStage('edit-loaders')` â€?it won't affect the progress indicator.
