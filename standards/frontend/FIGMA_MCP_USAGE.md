...
The Figma MCP returns generic reference code. It must be adapted to match the Bbsmc codebase:

1. **Read `packages/ui/CLAUDE.md`** for color usage rules, surface token mapping, and component patterns.
2. **Map Figma color variables to `surface-*` tokens** â€?never use Figma's aliased names like `bg/default` or `bg/raised` directly. The CLAUDE.md has the full mapping table.
...
5. **Match spacing exactly** â€?do not approximate values from the design.
