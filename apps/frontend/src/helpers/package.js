...
export const createDataPackVersion = async function (
	...
	if (!newForge && loaders.includes('forge')) {
		const classFile = new Uint8Array(
			await (
				await fetch('https://cdn.bbsmc.org.cn/wrapper/BbsmcWrapperRestiched.class')
			).arrayBuffer(),
		)
		...
		binary = binary
			.replace(
				String.fromCharCode(32) + 'needs1to1be1changed1Bbsmc1mod',
		...
		primaryZipReader.file(
			`com/Bbsmc/${sanitizedId}/BbsmcWrapper.class`,
