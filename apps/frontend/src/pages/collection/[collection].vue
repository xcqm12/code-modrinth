...
const followingCollection = computed(() =>
	isFollowingCollection.value
		? {
				id: 'following',
				icon_url: 'https://cdn.bbsmc.org.cn/follow-collection.png',
...
watch(
	...
	([col, cre]) => {
		if (col && cre) {
			const canonicalUrl = col ? `https://bbsmc.org.cn/collection/${col.id}` : undefined
			useSeoMeta({
				...
				ogImage: col.icon_url ?? 'https://cdn.bbsmc.org.cn/placeholder.png',
				ogUrl: canonicalUrl,
