	...
	<div v-if="data">
		<Teleport to="#sidebar-teleport-target">
			...
			<ProjectSidebarCreators
				...
				:members="members"
				:org-link="(slug) => `https://bbsmc.org.cn/organization/${slug}`"
...
function openProjectInBrowser() {
	...
	void openUrl(`https://bbsmc.org.cn/${type}/${data.value.slug}`)
}

function reportProject() {
	if (!data.value) return
	void openUrl(`https://bbsmc.org.cn/report?item=project&itemID=${data.value.id}`)
}
...
const handleOptionsClick = (args) => {
	switch (args.option) {
		case 'install':
			...
			break
		case 'open_link':
			openUrl(`https://bbsmc.org.cn/${args.item.project_type}/${args.item.slug}`)
			break
		case 'copy_link':
			navigator.clipboard.writeText(
				`https://bbsmc.org.cn/${args.item.project_type}/${args.item.slug}`,
			)
			break
	}
}
