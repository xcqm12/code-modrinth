...
function contentOwnerLink(owner: ContentOwner): NonNullable<ContentOwner['link']> {
	...
	return () => {
		void openUrl(`https://bbsmc.org.cn/organization/${owner.id}`)
	}
}
	...
	switch (format) {
		...
		case 'urls':
			text = source
				...
				.map((x) => `https://bbsmc.org.cn/${x.project_type}/${x.project?.slug}`)
				.join('\n')
			break
		case 'markdown':
			text = source
				.map((x) => {
					...
					if (x.project?.slug) {
						return `[${name}](https://bbsmc.org.cn/${x.project_type}/${x.project.slug})`
					}
					return name
				})
				.join('\n')
		...
		options.push({
			...
			action: async () => {
				await navigator.clipboard.writeText(
					`https://bbsmc.org.cn/${item.project_type}/${item.project?.slug}`,
				)
			},
		})
	}
