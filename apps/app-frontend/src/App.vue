...
async function setupApp() {
	...
	fetch(`https://api.bbsmc.org.cn/appCriticalAnnouncement.json?version=${version}`)
		...
		.catch(() => {
			console.log(
				`No critical announcement found at https://api.bbsmc.org.cn/appCriticalAnnouncement.json?version=${version}`,
			)
		})

	fetch(`https://bbsmc.org.cn/news/feed/articles.json`)
		.then((response) => response.json())
		.then((res) => {
			if (res && res.articles) {
...
async function checkLinuxUpdates() {
	try {
		const [response, currentVersion] = await Promise.all([
			fetch('https://launcher-files.bbsmc.org.cn/updates.json'),
			getVersion(),
		])
					...
					<div v-if="news && news.length > 0" class="p-4 flex flex-col items-center">
						...
						<div class="space-y-4 flex flex-col items-center w-full">
							...
							<ButtonStyled color="brand" size="large">
								<a href="https://bbsmc.org.cn/news" target="_blank" class="my-4">
									<NewspaperIcon />
