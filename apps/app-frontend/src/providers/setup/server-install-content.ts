import type { AbstractmodrinthClient, Archon, Labrinth } from '@bbsmc/api-client'
import {
...
function getQueuedInstallOwnerFallback(project: InstallableSearchResult) {
	if (project.organization) {
		...
		return {
			...
			link: `https://bbsmc.org.cn/organization/${ownerId}`,
		}
	}
		...
		if (project.organization) {
			...
			if (organization) {
				return {
					...
					link: `https://bbsmc.org.cn/organization/${organization.slug}`,
				}
			}
		}
