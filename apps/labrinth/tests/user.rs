    ...
    with_test_environment(
        ...
        |test_env: TestEnvironment<ApiV3>| async move {
            sqlx::query!(
                ...
                VALUES
                    (1000, 'userland', 'userland@bbsmc.org.cn', 'developer'),
                    (1001, 'useful', 'useful@bbsmc.org.cn', 'developer'),
                    (1002, 'Useless', 'useless@bbsmc.org.cn', 'developer')
                ",
            )
        ...
        |test_env: TestEnvironment<ApiV3>| async move {
            for i in 0..30 {
                sqlx::query!(
                    ...
                    2000 + i,
                    format!("prefix{i:02}"),
                    format!("prefix{i:02}@bbsmc.org.cn"),
                )
                .execute(&*test_env.db.pool)
                .await
                .unwrap();
            ...
            sqlx::query!(
                "
                INSERT INTO users (id, username, email, role)
                VALUES (2100, 'prefix_under_score', 'prefix_under_score@bbsmc.org.cn', 'developer')
                ",
            )
            .execute(&*test_env.db.pool)
            .await
