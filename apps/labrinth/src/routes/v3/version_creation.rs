...
pub async fn upload_file(
    ...
    if other_file_names.contains(&format!("{file_name}.{file_extension}")) {
        return Err(CreateError::InvalidInput(
            "Duplicate files are not allowed to be uploaded to Bbsmc!"
                .to_string(),
        ));
    }
    ...
    let data = read_from_field(
        field, 1024 * (1 << 20),
        "Project file exceeds the maximum of 1024MiB. Contact a moderator or admin to request permission to upload larger files."
    ).await?;
